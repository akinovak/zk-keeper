...
export default class SemaphoreService extends GenericService {
    async genProof(identity: ZkIdentity, request: ISemaphoreProofRequest): Promise<ISafeProof> {
        const { circuitFilePath, zkeyFilePath, merkleServiceAddress, externalNullifier, signal } = request;

        console.log('Commitment ovde je: ', identity.genIdentityCommitment())

        const response: AxiosResponse = await axios.post(merkleServiceAddress, {
